...
export class GestionPlatService {
  ...
  updatePlat(platId: any, updatedPlatData: any): Observable<any> {
    const headers = this.getHeaders();
    return this.http.put<any>(`${this.platsUrl}/update/${platId}`, updatedPlatData, { headers });
  }
  ...
  archiver(platId: number): Observable<any> {
    const headers = this.getHeaders();
    return this.http.patch<any>(`${this.platsUrl}/archiver/${platId}`, {}, { headers });
}

getArchivedPlats(): Observable<any[]> {
  const headers = this.getHeaders();
  return this.http.get<any[]>(`${this.platsUrl}/archived`, { headers });
}
desarchiver(platId: number): Observable<any> {
  const headers = this.getHeaders();
  return this.http.patch<any>(`${this.platsUrl}/desarchiver/${platId}`, {}, { headers });
}
// /plat/archiver/{id}
