...
export class PanierService {
  private _nombreArticlesDansPanier: Subject<number> = new Subject<number>();
  public nombreArticlesDansPanier = this._nombreArticlesDansPanier.asObservable();


  constructor(private http: HttpClient ,private router:Router) { this.chargerPanier(); }

  private panier: any[] = [];
  private nombreArticles: any = ""; // Nouvelle propriété
  ...
  ajouterAuPanier(plat: any, quantite = 1) {
    ...
    this._nombreArticlesDansPanier.next(this.panier.reduce((total, item) => total + item.quantite, 0));
